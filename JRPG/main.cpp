...
SDL_Texture* LoadTexture(std::string filePath, SDL_Renderer* renderTarget)
{
	...
	{
		//SDL_SetColorKey(surface, SDL_TRUE, SDL_MapRGB(surface->format, 255, 0, 0)); // HERE
...
int main(int argc, char* argv[])
{
	...
	window = SDL_CreateWindow("SDL made easy", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1024, 768, SDL_WINDOW_SHOWN);
	renderTarget = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	currentImage = LoadTexture("rects.png", renderTarget);
	...
	TTF_Font* font = TTF_OpenFont("ModernChristmas.otf", 80);
	SDL_Color color;
	color.r = 0;
	color.b = 0xff;
	color.g = 0;
	...
	SDL_SetRenderDrawColor(renderTarget, 0x1f, 0x1f, 0, 0xff);
	...
	while (isRunning)
	{
		...
		currentTime = SDL_GetTicks(); // miliseconds since SDL initialization
		...
		if (frameTime >= 0.25f) // 4 times per second
		{
			frameTime = 0;
	...
	TTF_Quit(); // Забыли в прошлом видео
	IMG_Quit(); 
	SDL_Quit();
